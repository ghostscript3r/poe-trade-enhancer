...
function addChaosEquiv(item, doSort) {
  ...
  if (item.hasClass('displayoffer')) {
    ...
    if (buyout !== 0) {
      ...
      var tip = getChaosEquivTip(item.data().buyout, translated);
      tippy(sortHandle[0], {content: tip[0]});
...
function addMaxQtCost(item) {
  if ((item.find(".gem-progress").length > 0 || item.find("img[src*='/2DItems/Gems/']").length > 0) && !(item.find(".corrupted").length > 0 && item.data().quality < 20)) {
...
var openSearch = function(search, newTab, target) {
  var form = $(`<form id="poeTradeOpener${target ? '-'+target : ''}" class="hide" action="http://poe.trade/search" method="post" ${(newTab ? 'target="_blank"' : (target ? 'target="'+target+'"' : ''))}></form>`);
  $('body').append(form);
...
var openMultiSearch = function(searches) {
  ...
  var getDeferredSearchId = function(search, retry) {
    ...
    var success = function(response, text, jqXHR) {
      debug("getDeferredSearchId.DONE FETCHING: ", jqXHR, jqXHR.getAllResponseHeaders(), xhr, xhr.responseURL);
  ...
  $.each( searches, function( i, search ) {
    fetches.push(getDeferredSearchId(search, 3));
  });
